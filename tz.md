...
3.2. Фильм
  - 3.2.1. Фильм представлен набором информации:
    ...
    - [x] Дата публикации фильма. Обязательное. По умолчанию — текущая дата.
    ...
    - [x] Рейтинг. Обязательное. Рассчитывается автоматически на основе оценок пользователей (средний бал). Значение по умолчанию — 0 (после создания фильма).
...
### 4. Функциональность
  ...
  - [x] 4.3.3. Список всех переменных окружения приведён в файле Workflow.md, который расположен в корне проекта. Формат описания: PARAM=value — описание.
...
### 5. Список ресурсов
  ...
  - [x] 5.1.3. Для добавления нового фильма клиент передаёт информацию, указанную в пункте 3.2, за исключением значений, которые рассчитываются автоматически.
  ...
  - [x] 5.3.2. При попытке клиентом удалить фильм другого пользователя сервис возвращает ошибку и корректный код состояния.
  ...
  - [x] 5.15.3. Если клиент авторизован, сервис возвращает соответствующий код состояния и информацию о пользователе (смотри пункт 3.1). Информация о пользователе не должна содержать конфиденциальные данные (пароль).
...
- [x] 9.6. В коде не должно быть зашито данных, которые могут меняться при переносе в продуктовую среду. Например: доступы для подключения к СУБД, адрес сервера для получения тестовых данных и так далее. Все перечисленные данные должны устанавливаться через переменные окружения.
